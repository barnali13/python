...
#31.User Authentication System  If email exists If password matches Else show proper error message.
users = {"admin@example.com": "pass123", "user@example.com": "mypass"}
...
#32.Role-Based Access Control   Admin → full access Editor → limited access User → read-only
role = input("Enter your role (admin/editor/user): ")
if role == "admin":
...
#33.API Request Validation  Check if token exists   Check if token is valid Return status codes (200, 401, 403)
